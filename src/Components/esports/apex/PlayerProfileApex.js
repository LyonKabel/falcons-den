...
import ApexTeam from './ApexRoster';
import '../../../Styling/gameCSS/PlayerProfile.css'; 


const PlayerProfile = () => {
  const { IGN } = useParams();
  const player = Object.values(ApexTeam).find(
    (p) => p.IGN.toLowerCase() === IGN.toLowerCase()
  );

  if (!player) {
    console.log("Player not found", { IGN }, player, ApexTeam);
